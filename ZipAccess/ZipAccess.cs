    ...
    public class ZipAccess : IDisposable
    {
        ...
        public void SevenZipAccessorInit()
        {
            string dllPath = Path.Combine(Path.GetDirectoryName(
              (new System.Uri(System.Reflection.Assembly.GetExecutingAssembly().CodeBase)).AbsolutePath), @"7z32.dll");
            if (Environment.Is64BitOperatingSystem)
            {
                dllPath = dllPath.Replace(@"7z32.dll", @"7z64.dll");
            }
            SevenZipBase.SetLibraryPath(HttpUtility.HtmlEncode(dllPath).Replace("%20", " "));
        }
